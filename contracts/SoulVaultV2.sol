...
library Address {
     ...
     * taken to not create reentrancy vulnerabilities. Consider using Reentrancy Guard.
     */
 ...
 * @dev Interface of the ERC20 standard as defined in the EIP.
 */
interface ISoulERC20 is IERC20 {
    function mint(address to, uint256 amount) external;
}


contract SoulVault is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Soul to distribute per block.
        uint256 lastRewardBlock; // Last block number that Soul distribution occurs.
        uint256 accSoulPerShare; // Accumulated Soul per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 harvestInterval; // Harvest interval in seconds
        uint256 totalLp; // Total token in Pool
        uint256 lockupDuration; // time the user will be locked in the pool after depositing or claiming rewards
    }

    ISoulERC20 public soul;
    ...
    // Total Soul in Soul Pools (can be multiple pools)
    uint256 public totalSoulInPools = 0;
    ...
    constructor(ISoulERC20 _soul, uint256 _soulPerBlock) {
    ...
    // View function to see pending Soul on frontend.
        ...
        returns (uint256)
    {
        ...
        uint256 pending = user.amount.mul(accSoulPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    // View function to see if user can harvest Soul.
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSoulPerShare = pool.accSoulPerShare.add(
            soulReward.mul(1e12).div(pool.totalLp)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to SoulVault for Soul allocation
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(
            block.number >= startBlock,
            "SoulVault: cannot deposit before farming start"
        );
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(soul)) {
                totalSoulInPools = totalSoulInPools.add(_amount);
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(soul)) {
                totalSoulInPools = totalSoulInPools.sub(_amount);
            }
    ...
    // Pay or lockup pending Soul.
    function payOrLockupPendingSoul(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        uint256 pending = user.amount.mul(pool.accSoulPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (canHarvest(_pid, _msgSender())) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeSoulTransfer(_msgSender(), totalRewards);
            }
        } else if (pending > 0) {
    ...
    // Safe Soul transfer function, just in case if rounding error causes pool do not have enough Soul.
    function safeSoulTransfer(address _to, uint256 _amount) internal {
        if (soul.balanceOf(address(this)) > totalSoulInPools) {
            //SoulBal = total Soul in SoulVault - total Soul in Soul pools, 
                // this will make sure that SoulVault never transfer rewards from deposited Soul pools.
            uint256 SoulBal = soul.balanceOf(address(this)).sub(
                totalSoulInPools
            );
            if (_amount >= SoulBal) {
    ...
    function updateEmissionRate(uint256 _soulPerBlock) public onlyOperator {
        massUpdatePools();
    ...
    function enableMetaTxns() public onlyOperator {
        require(!metaTxnsEnabled, "SoulVault: meta transactions are already enabled");
    ...
    function disableMetaTxns() public onlyOperator {
        require(metaTxnsEnabled, "SoulVault: meta transactions are already disabled");

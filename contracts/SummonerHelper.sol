...
contract SummonerHelper is Ownable {
    ...
    // function inactiveLength() public view returns (uint length) {
    //     length = inactivePools.length;
    // }

    function getInactivePools() public view returns (uint[] memory) {
        return Inactive;
    }

    function isPoolActive(uint pid) public view returns (bool) {
        bool active = true;
            for ( uint index = 0; index < poolLength(); index++ ) {
                if ( pid == Inactive[index] ) {
                    active = false;
                }
            }
        return active;
    ...
    function getInactiveIndex(uint pid) public view returns (uint index) {
            for ( index = 0; index < poolLength(); index++ ) {
                if ( Inactive[index] == pid) {
                    return index;
                }
            }
    }
    ...
    function getInactiveUserPools(address account) public view returns (uint inactiveDeposits) {
        
        for(uint index = 0; index < poolLength(); index++) { 
            if ( getUserPayout(index, account) > 0 && !isPoolActive(index)) {
    ...
    // function getUserInactivePools(uint pid, address account) public view returns (uint) {
    //     payout = SummonerContract.pendingSoul( pid, account );
    // }

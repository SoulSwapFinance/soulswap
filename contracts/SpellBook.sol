...
// Copyright (c) 2021 theeEnchantress - All Rights Reserved
// Twitter: @0xBuns
...
interface IStrategy {
    ...
    // The actualAmount should be very close to the amount. 
...
library PossessedERC20 {
    ...
    function safeTransfer(
        ...
        require(
            ...
            "PossessedERC20: Transfer failed"
        );
    }
    ...
    function safeTransferFrom(
        ...
        require(
            success && (data.length == 0 || abi.decode(data, (bool))),
            "PossessedERC20: TransferFrom failed"
        );
    }
}
...
library PossessedMath {
    ...
    function to128(uint256 a) internal pure returns (uint128 c) {
        require(a <= type(uint128).max, "PossessedMath: uint128 Overflow");
        c = uint128(a);
    }

    function to64(uint256 a) internal pure returns (uint64 c) {
        require(a <= type(uint64).max, "PossessedMath: uint64 Overflow");
        c = uint64(a);
    }

    function to32(uint256 a) internal pure returns (uint32 c) {
        require(a <= type(uint32).max, "PossessedMath: uint32 Overflow");
        c = uint32(a);
    }
}

library PossessedMath128 {
...
library RebaseLibrary {
    using PossessedMath for uint256;
    using PossessedMath128 for uint128;
...
// Source: Ownable.sol + Claimable.sol
// Edited by the Enchantress
...
contract PossessedOwnable is PossessedOwnableData {
    ...
    function transferOwnership(
        ...
        if (direct) {
            // Checks
            require(
                newOwner != ZERO_ADDRESS || renounce,
...
contract PossessedFactory {
    ...
    function deploy(
        ...
        require(
            ...
            "PossessedFactory: No masterContract"
        );
...
contract MasterContractManager is PossessedOwnable, PossessedFactory {
                ...
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            ...
                            approved
                                ? "Give FULL access to funds in (and approved to) SpellBook?"
                                : "Revoke access to SpellBook?",
                            user,
                            masterContract,
                            approved,
                            nonces[user]++
...
contract BasePossessedBatchable {
    ...
    // F2: Calls in the batch may be payable, delegatecall operates in the same context, 
        // each call in the batch has access to msg.value
...
contract PossessedBatchable is BasePossessedBatchable {
...
// File contracts/SpellBook.sol
// License-Identifier: UNLICENSED

/// @title SpellBook
/// @author thee Enchantress
/// @notice The SpellBook is a vault for tokens. The stored tokens can be flash loaned and used in strategies.
...
contract SpellBook is MasterContractManager, PossessedBatchable {
    using PossessedMath for uint256;
    using PossessedMath128 for uint128;
    ...
    struct StrategyData {
        ...
        uint128 balance; // the balance of the strategy that SpellBook thinks is in there
    }
    ...
    // If 'from' is the SpellBook itself, it's allowed. Any ETH, token balances
        // (above the known balances) or SpellBook balances can be taken by anyone.
    // This is to enable skimming, not just for deposits
        // but also for withdrawals or transfers, enabling better composability.
    ...
    modifier allowed(address from) {
        if (from != msg.sender && from != address(this)) {
            ...
            require(
                masterContract != address(0),
                "SpellBook: no masterContract"
            );
            require(
                masterContractApproved[masterContract][from],
                "SpellBook: Transfer not approved"
            );
        }
        _;
    }
    ...
    {
        // Checks
        require(to != address(0), "SpellBook: to not set"); // To avoid a bad UI from burning funds
        ...
        if (share == 0) {
            ...
            // amount may be lower than the value of share due to rounding
                // in that case, add 1 to amount (Always round up)
        ...
        // During flashloans the _tokenBalanceOf is lower than 'reality', 
            // so skimming deposits will mostly fail during a flashloan.
        require(
            from != address(this) ||
                token_ == USE_ETHEREUM ||
                amount <= _tokenBalanceOf(token).sub(total.elastic),
            "SpellBook: Skim too much"
        );
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            // X2: If the WETH implementation is faulty or malicious, 
                // it will block adding ETH (but we know the WETH implementation)
    ...
    function withdraw(
        ...
        Rebase memory total = totals[token];
        if (share == 0) {
            // value of the share paid could be lower than the amount paid due to rounding, 
                // in that case, add a share (Always round up)
        ...
        // There have to be at least 1000 shares left to prevent resetting the share/amount ratio 
            // (unless it's fully emptied)
        require(
            total.base >= MINIMUM_SHARE_BALANCE || total.base == 0,
            "SpellBook: cannot empty"
        );
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            require(success, "SpellBook: ETH transfer failed");
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "SpellBook: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    /// @param borrower The address of the contract that implements 
        // and conforms to `IFlashBorrower` and handles the flashloan.
    ...
    /// @param borrower The address of the contract that implements 
        // and conforms to `IBatchFlashBorrower` and handles the flashloan.
    ...
    function setStrategy(IERC20 token, IStrategy newStrategy) public onlyOwner {
        ...
        if (data.strategyStartDate == 0 || pending != newStrategy) {
            ...
            // C1 - All math done through PossessedMath (SWC-101)
    ...
    // F5 - Checks-Effects-Interactions pattern followed? (SWC-107)
    // F5: Total amount is updated AFTER interaction. But strategy is under our control.
    // F5: Not followed to prevent reentrancy issues with flashloans and SpellBook skims?
    function harvest(
        IERC20 token,
        bool balance,
        ...
        if (balanceChange > 0) {
            ...
            // But tokens with balances that large are not supported by the SpellBook.
            uint256 sub = uint256(-balanceChange);
